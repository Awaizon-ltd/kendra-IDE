...
type Cell = { status: 'loading' } | { status: 'ok'; text: string } | { status: 'error'; text: string };

/** Every zero-argument view function, read in one go — a live snapshot of contract state. */
export default function StatePanel({ fns, target, compact = false }: { fns: ContractFn[]; target: Target | null; compact?: boolean }) {
    ...
    await Promise.all(
      readable.map(async (f) => {
        try {
          ...
          setCells((c) => ({ ...c, [f.id]: { status: 'ok', text } }));
        } catch (e) {
  ...
  }, [target, readable.map((f) => f.id).join()]);
